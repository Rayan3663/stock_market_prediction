...
#j = 0;
# for i in range(0,1):
dataframes[['Daily Return']].plot(linestyle='--', marker='o').set_title(f"{stock_name}");
plt.show()
...
def create_testing_data_set(model, scaler, training_data_len, test_data_len):
   ...
   # print()
    ...
    print('mean_predictions', np.mean(predictions))
...
# Test the function
TEST_DATA_LENGTH = 60
x_test, y_test, predictions, rmse, predicted_main_out  = create_testing_data_set(lstm_model, scaler, training_data_len, TEST_DATA_LENGTH)
